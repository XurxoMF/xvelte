...
- [Keyboard behavior and lifecycle](#keyboard-behavior-and-lifecycle)
...
Copy `shortcut.ts` and install Svelte 5 if the project does not already provide it:
...
The attachment uses Svelte's `Attachment` type and the `{@attach ...}` syntax. It requires no global CSS, theme variables, icons, `$lib/utils` helpers, xvelte components, hooks, localization messages, or additional files.
...
| `shortcut.md` | Installation, usage, API, lifecycle, and accessibility guide.                |

`shortcut.ts` and its exported declarations are the source of truth for the public API.
