...
A static callout for drawing attention to important information, outcomes, warnings, or errors. It provides default and destructive visual variants, optional icon-aware layout, structured title and description parts, and a positioned action area.
...
Use `variant="destructive"` for errors or dangerous outcomes. You provide any icons, and they should be decorative when the adjacent text communicates the same meaning.
...
| ---------- | ---------------------------- | ----------- | ----------------------------------------------------------------------------------------------- |
| `variant`  | `"default" \| "destructive"` | `"default"` | Selects the semantic color treatment generated by `rootVariants`.                               |
...
The destructive root variant changes the description color through its `data-slot`. Paragraphs except the last receive a bottom margin, and descendant links inherit the local underline and hover treatment.
...
| `Description` | `alert-description` | Receives the destructive description color through the root variant.             |
...
Alert has no built-in user-facing copy or localization messages. Your app supplies and translates all titles, descriptions, action labels, and accessible icon names. Do not translate the technical `default`/`destructive` variant names or `data-slot` values.
...
Alert uses the `card`, `card-foreground`, `destructive`, `muted-foreground`, `foreground`, and `border` semantic color utilities. Your theme must define and expose these tokens; the values below are xvelte's light defaults:
...
Define equivalent values inside the application's dark selector if it supports a dark theme. Alert requires no global keyframes, animation utilities, or component-specific CSS variables.

### Icons

Alert does not import or require an icon. Any icon component you place directly inside it can use the built-in SVG layout.

The destructive example above uses xvelte's semantic icon facade. Copying that example requires `@tabler/icons-svelte` and this export in `$lib/icons`:
