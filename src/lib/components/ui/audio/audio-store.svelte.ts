...
import { type HtmlAudio, type Track } from "$lib/components/ui/audio/html-audio";
...
export class AudioStore {
	...
	addTracksToEndOfQueue(tracks: Track[]): void {
		...
		const ids = new SvelteSet(this.queue.map((s) => s.id));
	...
	loadAndPlayTrack(track: Track, queueIndex: number): void {
		const isLiveStream = track.live === true || (track.duration !== undefined && this.htmlAudio.isLive(track.duration));
