<script lang="ts" module>
	...
	export type RootProps = {
		tracks?: Track[];
		/** Local-storage key. Set to `null` to disable persistence. */
...
<script lang="ts">
	...
	import { AudioStore, calculateNextIndex, setAudioContext } from "$lib/components/ui/audio/audio-store.svelte";
	import { HtmlAudio } from "$lib/components/ui/audio/html-audio";

	let { tracks = [], storageKey = "audio:ui:store", children }: RootProps = $props();
	...
	$effect(() => {
		...
		const changed = q.length === 0 || q.length !== t.length || q.some((track, i) => track.id !== t[i]?.id);
		if (changed) {
			audioStore.queue = [...t];
	...
	onMount(() => {
		...
		const handleError = async (e: Event) => {
			...
			audioStore.errorMessage =
				recoverable && errorRetryCount >= MAX_ERROR_RETRIES ? `Failed after ${MAX_ERROR_RETRIES} attempts: ${message}` : message;
		};
	...
	$effect(() => {
		const queueLength = audioStore.queue.length;
		if (queueLength === 0 && preloadAudio) preloadAudio.src = "";
	});
