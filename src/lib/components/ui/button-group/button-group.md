...
Follow the Separator component's unit guide to install it and understand its API. Button Group itself requires no other xvelte component.

The examples also use the Button and Input components. Copy only the components your interface uses and follow each component's unit guide to install it; Button Group does not import them internally.
