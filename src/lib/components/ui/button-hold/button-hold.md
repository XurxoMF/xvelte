...
Hold Button wraps the local Button component. The table documents every Hold Button-owned prop and the inherited Button options most relevant to normal use. Follow the Button component's unit guide for its complete variants, sizes, native attributes, form behavior, styling, and dependencies. Hold Button's `index.ts`, exported types, and local source are the source of truth.
...
Inherited Button focus, invalid, pressed, and expanded styling does not create those behaviors. Follow the Button unit guide for the complete native and ARIA obligations.
...
- `index.ts`
- `button-hold.md`
  Follow the Button component's unit guide to install it and understand its variants, sizes, native API, styling, accessibility, and theme requirements. Hold Button imports `Root` and `RootProps` through Button's public `index.ts`.
...
| `button-hold.md`          | Composition, examples, API, gesture lifecycle, styling, accessibility, localization, dependencies, limitations, and credits. |
