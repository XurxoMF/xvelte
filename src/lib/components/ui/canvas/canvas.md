...
Follow the Button component's unit guide to install it and understand its API. Canvas requires no other xvelte UI component, hook, or attachment. Keep `canvas-context.svelte.ts` with the Canvas component files because it provides their shared state and exported coordinate helpers.
