...
Follow the Button component's unit guide to install it and understand its API. Carousel requires no other xvelte UI component, hook, or attachment. Keep `carousel-context.ts` with the Carousel component files because it provides their shared state and public Embla types.
