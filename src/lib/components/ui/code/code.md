...
Follow the Button component's unit guide to install its complete API, styling, and theme requirements. `Root` itself can render without Button when neither `CopyButton` nor `Overflow` is used.

`Root` requires the complete `src/lib/components/ui/scroll-area` folder. Follow the Scroll Area component's unit guide to install its Root, Viewport, fixed-axis scrollbars, internal context, styling, and Bits UI dependency.

### Global styles

Copy Tailwind's import and the semantic variables and mappings below into the app's global stylesheet. These are xvelte's defaults; replace their values with the app's theme while preserving the names. The required Button unit guide adds the extra tokens used by its controls.
...
| `code.md`                 | Documents composition, dynamic languages, API, behavior, and installation requirements.            |
