	...
	<div class="flex items-center gap-3">
		...
		<div class="flex flex-1 flex-col justify-center gap-3">
			...
			{#if allowOpacity}
				...
				>
					<div class="absolute inset-0 rounded-full" style:background={`linear-gradient(to right, transparent, ${hsvToHex(h, s, v, 1)})`}></div>
					<div
						class="pointer-events-none absolute top-1/2 h-3 w-3 -translate-x-1/2 -translate-y-1/2 rounded-full bg-white"
						style:left={`${a * 100}%`}
					></div>
						...
						<Command.List>
							<Command.Group>
								{#each ["hex", "rgb", "hsl", "oklch"] as fmt (fmt)}
									<Command.Item value={fmt} onSelect={() => setFormat(fmt as ColorFormat)} class="flex h-7 justify-center text-[10px]">
										{fmt.toUpperCase()}
									</Command.Item>
								{/each}
							</Command.Group>
