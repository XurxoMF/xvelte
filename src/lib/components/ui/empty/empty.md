...
Empty requires no other xvelte component, hook, attachment, context file, localization message, Paraglide setup, shared style, external asset, or browser API. Buttons, avatars, input groups, illustrations, and popup containers are optional app compositions; copy each component and follow its unit guide only when it is used.
...
| `empty.md`                 | Installation, composition, examples, API, styling, accessibility, localization, dependencies, and credits. |
