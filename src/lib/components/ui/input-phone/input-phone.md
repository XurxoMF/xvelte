...
<div class="grid max-w-sm gap-1.5">
	...
	{#if !valid}
		<p class="text-sm text-danger" role="alert">Enter a valid phone number.</p>
	{/if}
</div>
```
...
Copy these complete xvelte components and follow their unit guides:

- `src/lib/components/ui/input`: `input-root.svelte`, `index.ts`, and `input.md`.
- `src/lib/components/ui/combobox`: every Svelte file, `combobox-context.svelte.ts`, `index.ts`, and `combobox.md`.
- `src/lib/components/ui/button-group`: every Svelte file, `index.ts`, and `button-group.md`.

Follow the Combobox and Button Group unit guides to copy and configure their own required components. Button Group is needed for the standard joined layout, but Root also supports app-owned layouts without it.
...
| `input-phone.md`                      | Composition, examples, API, styling, accessibility, localization, dependencies, and credits.   |
