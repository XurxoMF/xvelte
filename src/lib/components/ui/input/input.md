...
- `index.ts`
- `input.md`
  The examples use native labels and buttons, so no additional xvelte component is required. Follow the relevant component's unit guide separately when choosing to compose Input with Label, Button, Field, Input Group, or another xvelte component.
...
| `input.md`          | Usage, examples, API, bindings, styling, accessibility, localization, dependencies, limitations, and credits.                      |
