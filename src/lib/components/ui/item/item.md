...
Copy the Separator component with Item. Its folder does not yet contain a colocated unit guide, so the installation requirements Item needs from it are included here: the two files above, `bits-ui`, the shared `cn` helper, and the semantic `border` color defined below. Use the exported Separator types and `index.ts` as its current API reference.
...
| `item.md`                 | Installation, API, behavior, and usage guide.                         |
