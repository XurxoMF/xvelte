...
- `index.ts`
- `radio-group.md`
  Radio Group requires no other xvelte component, hook, attachment, context module, localization message, shared style file, font, image, or external network service. The examples use native labels, so the separate Label component is optional; use it and follow its unit guide when consistent label styling is desired.
...
| `radio-group.md`          | Composition, examples, API, interaction, styling, accessibility, dependencies, and credits.     |
