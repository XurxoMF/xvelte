...
Sheet requires its own `CloseIcon`; follow Sheet's unit guide for that dependent component.
...
Copy these components and follow each available colocated unit guide for its own API and installation requirements:
...
| `sidebar.md`                                                               | Composition, examples, complete API, responsive behavior, accessibility, localization, styling, and installation guide. |
