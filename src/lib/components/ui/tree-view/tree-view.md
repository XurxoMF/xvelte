...
Default icons are 1rem. Folder's `class` affects only its trigger. File and Root classes merge through `cn()`. Collapsible contributes open/closed state attributes and behavior documented in its unit guide.
...
This is Tree View's direct token. Follow Collapsible's unit guide for its animation, focus, and additional theme requirements.
...
Copy Collapsible and follow its unit guide:
...
| `tree-view.md`            | Composition, examples, API, limitations, accessibility, styling, and installation guide.           |
