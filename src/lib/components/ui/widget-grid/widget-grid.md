...
- `src/lib/components/ui/button`: `button-root.svelte`, `index.ts`, and `button.md`. Follow Button's unit guide for its full package, token, and utility setup.
...
Root imports all GridStack-specific structural CSS, prevents its private Item wrapper from creating an outer scrollbar, and restyles its interaction placeholder with `--muted` and `--radius`. WidgetGrid's handles use Button's `muted`, `muted-foreground`, and `foreground` semantic colors. Copy and configure Button first; its unit guide includes the exact `layout.css` variables, `@theme` mappings, border base rule, and replaceable theme values required by the handles.
...
| `widget-grid.md`                   | Installation, composition, examples, API, styling, accessibility, localization, and dependencies.         |
